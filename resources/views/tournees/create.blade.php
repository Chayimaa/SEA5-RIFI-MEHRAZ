...
<head>
    ...
    <title>Nouvelle Tournée - Jardin de Cocagne</title>
...
<body class="bg-gray-50 min-h-screen">
    ...
    <main class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-900 mb-2">Créer une nouvelle tournée</h1>
            <p class="text-gray-600">Remplissez les informations et sélectionnez les dépôts à visiter</p>
        </div>
        ...
        <form action="{{ route('tournees.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="remarques" id="remarques" rows="3"
                                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent"
                                  placeholder="Notes ou instructions spéciales...">{{ old('remarques') }}</textarea>
                        @error('remarques')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div class="depot-item border-2 border-gray-200 rounded-lg p-4 hover:shadow-md transition-all">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" name="depots[]" value="{{ $depot->id }}" 
                                   ...
                                   {{ (is_array(old('depots')) && in_array($depot->id, old('depots'))) ? 'checked' : '' }}>
                            <div class="flex-1">
            ...
            <div class="flex justify-end space-x-4">
                ...
                <button type="submit" 
                    ...
                    <i class="fas fa-check mr-2"></i>Créer la tournée
                </button>
            </div>
        </form>
    </main>
